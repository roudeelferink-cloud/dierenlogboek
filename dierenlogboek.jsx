...
// ── Rob: pas APP_TITLE aan om de naam van je zoon erin te zetten, bijv. "Sems logboek" ──
...
// ── opslag (blijft bewaard tussen sessies) ──
async function loadIndex() {
  try {
    const r = await window.storage.get("spot:index");
    return r ? JSON.parse(r.value) : [];
  } catch { return []; }
}
async function saveIndex(list) {
  try { await window.storage.set("spot:index", JSON.stringify(list)); } catch (e) { console.error(e); }
}
async function loadPhoto(id) {
  try { const r = await window.storage.get("spot:photo:" + id); return r ? r.value : null; } catch { return null; }
}
async function savePhoto(id, url) {
  try { await window.storage.set("spot:photo:" + id, url); } catch (e) { console.error(e); }
}
async function deletePhoto(id) {
  try { await window.storage.delete("spot:photo:" + id); } catch (e) {}
}
...
// Stuurt de foto naar Claude en vraagt om diersoort, groep en Freek-stijl weetjes.
async function identifyAnimal(dataUrl) {
...
Regels: schrijf de weetjes in het Nederlands, kort en enthousiast, maximaal 1 zin per weetje, geschikt voor een kind van 8 tot 12 jaar. Eén emoji per weetje mag. Twijfel je? Geef dan je beste gok. Staat er geen dier op de foto, gebruik dan naam "", groep "anders" en weetjes ["Ik zie hier geen dier — maak een foto van het beestje zelf! 🔍"].`;
  ...
  return {
    ...
    groep: GROUP[parsed.groep] ? parsed.groep : "anders",
...
export default function App() {
    ...
    (async () => {
      ...
      for (const e of idx) {
        if (e.hasPhoto) {
          ...
          if (p) setPhotos((prev) => ({ ...prev, [e.id]: p }));
        }
      }
    })();
  }, []);
  ...
  async function addEntry(data, photoUrl) {
    const isNew = !speciesSet.has(data.name.trim().toLowerCase());
    const id = "s_" + Date.now() + "_" + Math.random().toString(36).slice(2, 7);
    const entry = { id, created: Date.now(), hasPhoto: !!photoUrl, ...data };
    const next = [entry, ...entries];
    setEntries(next);
    await saveIndex(next);
    if (photoUrl) {
      setPhotos((prev) => ({ ...prev, [id]: photoUrl }));
      await savePhoto(id, photoUrl);
    }
    setAdding(false);
    if (isNew) {
  ...
  async function removeEntry(id) {
    ...
    setPhotos((prev) => { const c = { ...prev }; delete c[id]; return c; });
    setDetail(null);
  }
  ...
  return (
    <div style={{ background: C.paper, color: C.ink, minHeight: "100vh", fontFamily: "'Nunito', ui-rounded, 'SF Pro Rounded', system-ui, sans-serif" }}>
      ...
      <main className="px-4 pb-28 max-w-md mx-auto">
        ...
        {!loaded ? (
          ...
          <div className="grid grid-cols-2 gap-3 mt-4">
            {visible.map((e) => (
              <SpotCard key={e.id} entry={e} photo={photos[e.id]} onClick={() => setDetail(e)} />
            ))}
          </div>
        )}
      </main>
      ...
      {detail && <DetailSheet entry={detail} photo={photos[detail.id]} onClose={() => setDetail(null)} onDelete={() => removeEntry(detail.id)} />}
